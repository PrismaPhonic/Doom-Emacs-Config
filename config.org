...
This snippet adds a hook to =org-mode= buffers so that =pmf/org-babel-tangle-config= gets executed each time such a buffer gets saved.  This function checks to see if the file being saved is the Emacs.org file you're looking at right now, and if so, automatically exports the configuration here to the associated output files.
...
This adds configuration for rust development setup, including auto formatting all buffers on save.
